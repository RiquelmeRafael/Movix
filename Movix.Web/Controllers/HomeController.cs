...
public class HomeController(IApiClient api) : Controller
{
    ...
    private async Task<CatalogPageVm> BuildVm(CatalogFilterVm filter, CancellationToken ct)
    {
        ...
        if (filter.PageSize > 2000) filter.PageSize = 2000; // limite superior de seguran�a.
    ...
    {
        // For�amos pageSize = 0 para pedir "todos os filmes" (reposit�rio interpreta 0 como 'no pagination').
        filter.Page = 1;
        filter.PageSize = 0;
